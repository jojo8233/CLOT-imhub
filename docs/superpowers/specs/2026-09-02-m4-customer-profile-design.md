...
采用“人工档案 + 最小审计”的垂直切片，而不是先做完整 M4 后端或先做模型自动提取。

原因如下：

1. 右侧面板和原生当前会话同步已经存在，可以用最小新增获得端到端业务价值；
2. 人工资料是后续自动提取的可信基线，先定义覆盖规则可以避免模型结果污染员工确认过的事实；
...
这张表在本切片只记录档案写入，不等同于完整审计能力；尤其不会顺带声称现有
`req.scoped.scope.requiresAudit` 已经覆盖 auditor 的所有读取。审计日志查询和敏感读取审计属于
后续 M4-2。
...
3. 切平台、账号或会话时取消旧请求，并丢弃迟到响应；
4. 读取成功后显示档案；未填写字段显示“尚未填写”；
5. 可写角色点击“手动补充”后进入六字段内联表单；auditor 只看到只读资料；
6. “取消”恢复最近一次服务器快照；“保存”提交完整字段和 snapshot revision；
7. 保存中禁止重复提交；成功后用响应替换 snapshot 和草稿；
8. `409` 时保留用户尚未保存的草稿，同时重新读取最新服务器 snapshot 并提示“档案已被其他人更新”；
   用户先对照新版本调整，再以新的 revision 保存，不能静默强制覆盖；
9. 网络或服务端失败保留当前编辑草稿并允许重试，不把失败草稿写入其他会话；
10. “重新提取”继续禁用并标注为后续 M4 能力。

### 6.2 人工资料优先

本切片只写人工资料。后续自动提取必须进入独立 suggestion/review 模型：模型建议只能由员工选择接受，
不能直接更新 `customer_profiles`，也不能覆盖非空人工字段。本切片不预建尚未使用的 suggestion 表。
...
- 更新 RUNBOOK 和阶段 checkpoint，不能把自动提取、告警或完整审计后台写成已完成。
...
2. M4-3 客户档案自动提取建议、人工接受/拒绝与来源标注；
3. M4-4 关键词规则、命中事件与管理员通知；
4. M4-5 团队和管理后台补齐；
5. M7 安装包、上游更新、GPL/AGPL 源码交付、部署和正式发布。
...
- 客户档案定向回归：6 个测试文件、43 个测试通过、0 失败；
- 全量类型检查：`pnpm typecheck` 退出码 0；
- 全量回归：78 个测试文件、648 个测试通过、1 个既有 todo、0 失败；
...
消息键、媒体引用、token、二维码、验证码或密钥。自动提取、完整审计查询、关键词告警、管理后台、
安装包交付和生产发布仍属于后续工作。
