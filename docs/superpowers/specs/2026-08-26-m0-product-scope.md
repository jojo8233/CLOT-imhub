...
简化聊天工具，而是在统一的 im-hub 外壳内承载平台原生客户端，同时把消息、客户档案和
权限数据回传到 im-hub 服务端。
...
M4-4 已实现只供公司内部使用的 owner 管理中心。唯一启用的 `owner` 管理员工、团队、唯一主管、
agent 调组和平台账号归属，并可执行首次强制改密、即时会话撤权和带再次验证的原子 owner 转让；
`manager`、`auditor`、`agent` 不能进入或调用管理功能。中心服务端与 PostgreSQL 是必要依赖，但不要求
向公网开放，也没有注册、邀请、邮件、短信、SSO 或客户入口。

本地平台 session/profile 不跨设备复制。Telegram/WhatsApp Web 只在支持能力的新版客户端按账号
分区自动清理；Signal 始终由公司在官方“已关联设备”中人工解除并在应用内确认。代码与自动化已完成，
真实数据库 migration、写开关灰度和 macOS/Windows 双平台人工验收仍待执行。
