...
日期：2026-08-29
状态：执行中；账本、报告、双真实账号 base/delete/edit 已验，媒体/回复组合探针已执行并
定位出向媒体时间语义；修正后真实 edit revision 和切换门槛待验
...
3. 补齐 TDLib 编辑、删除、媒体、回复和 remap 观测；不通过降低双方共同语义要求制造一致。
   ...
   媒体与文本时间仍严格比较。下一步编辑同一条 S5 caption 一次取得修正后的真实 revision，
   不新增消息，也不重做 M3-4 的媒体、回复或故障矩阵。
