...
日期：2026-08-29
状态：执行中；账本、报告、双真实账号 base/delete/edit 已验，媒体/回复自动覆盖已补；
真实媒体/回复 shadow 探针和切换门槛待验
...
3. 补齐 TDLib 编辑、删除、媒体、回复和 remap 观测；不通过降低双方共同语义要求制造一致。
   ...
   `source_local`。自动回归已完成，下一步只使用一条“带 caption 的图片并回复既有 S4”
   组合探针验证媒体+回复，不重做 M3-4 的媒体、回复或故障矩阵。
