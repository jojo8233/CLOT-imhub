...
日期：2026-09-05
状态：代码与自动化验证已完成；真实发布灰度和双平台人工验收待执行
...
- migration、preflight、RUNBOOK、权限矩阵、并发测试和桌面自动化；双平台人工验收清单待执行。
...
更换主管和归档团队的 preview 必须先枚举实际发生负责人变更的账号，将自动/人工清理数量纳入摘要；
在线旧版 Telegram/WhatsApp 客户端默认于 preview 阶段阻断。只有 owner 阅读影响并明确选择人工兜底后，
`allowManualCleanup` 才与当时的 revision 一起绑定进 operation token，execute 不得另行猜测。
...
公司已经从 Signal 官方设备列表解除对应旧设备。每个安装实例/待办必须单独展示原因和创建时间并逐项
确认；确认只关闭所选待办，不得声称 im-hub 擦除了 Signal profile。
...
- 人工清理待办按安装实例逐项展示安全元数据和确认按钮，不提供整个账号的一键全部完成。
...
`codex/m4-org-admin` 已完成第 1–12 项实现，并在隔离测试数据库上完成以下自动化验证：

- M4 定向测试：33 个测试文件、149 项测试通过；
- `pnpm typecheck` 通过；
- 全量 `pnpm test`：137 个测试文件、1042 项测试通过，todo 为 0；
- desktop production build 通过：main 28 个模块、preload 36 个模块、renderer 173 个模块。

整分支复审发现并修正了自助改密时的旧 WebSocket 即时撤权、Signal 历史连接模式的固定人工清理、
旧客户端人工兜底的显式确认、owner 转让后的多团队主管支持、结果未知界面状态和按安装实例逐项确认。
第二轮复审又补齐了改密撤权与 HTTP 结果丢失的乱序处理、单账号转移的显式人工兜底、团队换主管/归档的
清理预览与 token 绑定，以及 manager 全部团队的列表展示。
全量回归期间还发现旧关键词告警夹具使用了不合规的当前 manager membership；夹具已改为旧 manager 仅保留历史
recipient、当前新主管为唯一 lead，隔离回归和全量回归均通过。

本检查点没有对开发库或生产库运行 organization preflight / `0016` migration，没有打开
`ORGANIZATION_ADMIN_WRITES_ENABLED`，也没有修改真实员工、团队、平台账号或 owner。macOS 与 Windows
人工验收尚未执行，未发送新的真实平台消息。发布必须继续严格遵循第 12 节顺序；Signal 仍固定为官方
设备列表人工解除，各客户端各自确认，不能把应用内确认表述成自动清理 profile。
