...
describe('ScopedDb', () => {
  it('accounts() 已经带上 agent 的过滤条件', () => {
    const scoped = new ScopedDb(db, { kind: 'self', userId: 'u9' }, 'u9')
  ...
  it('manager 的两个入口都按 team 过滤', () => {
    const scoped = new ScopedDb(db, { kind: 'teams', teamIds: ['t1'] }, 'u9')
  ...
  it('没带组的 manager 从任何入口拿到的都是 where false', () => {
    const scoped = new ScopedDb(db, { kind: 'teams', teamIds: [] }, 'u9')
  ...
  it('keywordAlerts() 返回同时闭包当前 scope 与 actor 的告警仓储', () => {
    const scoped = new ScopedDb(db, { kind: 'self', userId: 'u9' }, 'u9')
    expect(scoped.keywordAlerts()).toBeInstanceOf(ScopedKeywordAlertRepo)
  })
})
