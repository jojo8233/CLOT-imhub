...
describe('production container runtime', () => {
  ...
  it('isolates edge and data services and fixes the direct proxy trust boundary', () => {
    ...
    const appNetworks = object(app.networks)
    const caddyNetworks = object(object(services.caddy).networks)

    expect(Object.keys(appNetworks)).toEqual(['data', 'edge'])
    expect(Object.keys(caddyNetworks)).toEqual(['edge'])
    expect(object(appNetworks.edge).ipv4_address).toBe('172.30.0.3')
    expect(object(caddyNetworks.edge).ipv4_address).toBe('172.30.0.2')
